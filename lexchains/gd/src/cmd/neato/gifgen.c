...
/*
    if (cstk[SP].pen != P_NONE) {
        for (i = 1; i < n; i++) {
            if (cstk[SP].pen == P_DASHED) {
                gdImageDashedLine(im, points[n].x, points[n].y,
                    points[n-1].x, points[n-1].y, cstk[SP].color_ix);
            } else {
                gdImageLine(im, points[n].x, points[n].y,
                    points[n-1].x, points[n-1].y, cstk[SP].color_ix);
            }
        }
        if (cstk[SP].pen == P_DASHED) {
            gdImageDashedLine(im, points[0].x, points[0].y,
                points[n].x, points[n].y, cstk[SP].color_ix);
        } else {
            gdImageLine(im, points[0].x, points[0].y,
                points[n].x, points[n].y, cstk[SP].color_ix);
        }
    }
*/
