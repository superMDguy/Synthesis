...
Tobj Tstring (char *s) {
    ...
    sp = Mnew (T_STRINGSIZE (strlen (s)), T_STRING);
    strcpy (&sp->s[0], s);
    return sp;
}

Tobj Tcode (Code_t *cp, long ci, long cl) {
    Tcode_t *codep;
    Code_t *cp2;
    char *s;
    long i, j, cn;

    codep = Mnew (T_CODESIZE (cl), T_CODE);
    cp2 = codep->c;
    for (i = 0; i < cl; i++) {
        switch (cp[i].ctype) {
        case C_INTEGER:
...
Tobj Ttable (long sizehint) {
    ...
    lp = Mallocate (sizehint * T_KVLISTPTRSIZE);
    tp->lp = lp;
    tp->ln = sizehint;
    tp->n = 0;
    tp->time = Ttime;
...
static void insert (Ttable_t *tp, Tobj ko, char *sk, Tobj vo) {
    ...
    if (!kvlp) {
        tp->lp[ind] = kvlp = Mallocate (T_KVLISTSIZE (1));
        kvlp->i = 0, kvlp->n = 1;
    } else if (kvlp->i == kvlp->n) {
        tp->lp[ind] = nkvlp = Mallocate (T_KVLISTSIZE (kvlp->n * 2));
        nkvlp->n = kvlp->n * 2;
        for (i = 0; i < kvlp->n; i++)
            nkvlp->kv[i] = kvlp->kv[i];
        nkvlp->i = kvlp->i;
        ((Mheader_t *) kvlp)->size = M_BYTE2SIZE (T_KVLISTSIZE (kvlp->n));
        Mfree (kvlp), kvlp = nkvlp;
    }
...
static void copytable (Ttable_t *tp, long ln) {
    ...
    lp = Mallocate (ln * T_KVLISTPTRSIZE);
    oln = tp->ln, tp->ln = ln;
    olp = tp->lp, tp->lp = lp;
    for (i = 0; i < ln; i++)
        lp[i] = NULL;
    for (i = 0; i < oln; i++) {
        ...
        for (j = 0; j < okvlp->i; j++) {
            ...
            if (!kvlp) {
                lp[ind] = kvlp = Mallocate (T_KVLISTSIZE (1));
                kvlp->i = 0, kvlp->n = 1;
            } else if (kvlp->i == kvlp->n) {
                lp[ind] = nkvlp = Mallocate (T_KVLISTSIZE (kvlp->n * 2));
                nkvlp->n = kvlp->n * 2;
                for (k = 0; k < kvlp->i; k++)
                    nkvlp->kv[k] = kvlp->kv[k];
                nkvlp->i = kvlp->i;
                ((Mheader_t *) kvlp)->size =
                        M_BYTE2SIZE (T_KVLISTSIZE (kvlp->n));
                Mfree (kvlp), kvlp = nkvlp;
            }
            kvlp->kv[kvlp->i].ko = kvp->ko, kvlp->kv[kvlp->i++].vo = kvp->vo;
        }
        ((Mheader_t *) okvlp)->size = M_BYTE2SIZE (T_KVLISTSIZE (okvlp->n));
        Mfree (okvlp);
    }
    ((Mheader_t *) olp)->size = M_BYTE2SIZE (oln * T_KVLISTPTRSIZE);
    Mfree (olp);
}
...
static void reccopytable (Ttable_t *fmtp, Ttable_t *totp) {
    ...
    totp->lp = Mallocate (fmtp->ln * T_KVLISTPTRSIZE);
    totp->ln = fmtp->ln;
    totp->n = fmtp->n;
    totp->time = Ttime;
    for (i = 0; i < totp->ln; i++) {
        ...
        totp->lp[i] = Mallocate (T_KVLISTSIZE (fmtp->lp[i]->n));
...
static void mapterm (void) {
    ...
    for (li = 0; li < MAPLISTN; li++) {
        ...
        for (cep = *lp; cep; cep = nep) {
            nep = cep->next;
            ((Mheader_t *) cep)->size = mapentrybyte2size;
            Mfree (cep);
        }
        *lp = NULL;
    }
