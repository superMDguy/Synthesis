...
#if 0 /* must fix */
            if (message == WM_LBUTTONUP && (WCU->bstate[1]))
#endif
...
LRESULT CALLBACK QueryWndProc (HWND hwnd,
        UINT message, WPARAM wparam, LPARAM lparam) {
    Gwidget_t *widget;
    WINDOWPOS *wpos;
    WORD l;
    ...
    if (Gnocallbacks || !(widget = findwidget (hwnd, G_QUERYWIDGET)))
        return (DefWindowProc(hwnd, message, wparam, lparam));
    switch (message) {
    case WM_WINDOWPOSCHANGED:
        wpos = (WINDOWPOS *) lparam;
        ...
        break;
    case WM_COMMAND:
        if (LOWORD (lparam) == 0 || !(wparam < Gwidgetn))
            return 0;
        if (HIWORD (lparam) == EN_CHANGE) {
            if ((n = SendMessage (widget->u.q->w, EM_GETLINECOUNT, 0, 0L)) < 2)
                return 0;
            *((WORD *) &Gbufp[0]) = Gbufn - 1;
            l = SendMessage (widget->u.q->w, EM_GETLINE, n - 1,
                    (LPARAM) (LPSTR) &Gbufp[0]);
            if (l != 0)
                return 0; /* no carriage return yet */
            *((WORD *) &Gbufp[0]) = Gbufn - 1;
            l = SendMessage (widget->u.q->w, EM_GETLINE, n - 2,
                    (LPARAM) (LPSTR) &Gbufp[0]);
            Gbufp[l] = 0;
            widget->u.q->state = 0;
        }
...
LRESULT CALLBACK ScrollWndProc (HWND hwnd,
    ...
    case WM_VSCROLL:
        ...
        case WM_HSCROLL:
            ...
            case SB_THUMBPOSITION: po.x = LOWORD (lparam);     break;
            case SB_THUMBTRACK:    po.x = LOWORD (lparam);     break;
        ...
        case WM_VSCROLL:
            ...
            case SB_THUMBPOSITION: po.y = LOWORD (lparam);     break;
            case SB_THUMBTRACK:    po.y = LOWORD (lparam);     break;
...
static void processcommand (Gwidget_t *widget, WPARAM wparam, LPARAM lparam) {
    ...
    if (LOWORD (lparam) == 0) { /* it's a menu */
        if (wparam != 999)
            menuselected = wparam;
        menupoped = FALSE;
        return;
    }
    if (!(wparam > 0 && wparam < Gwidgetn))
        return;
    child = &Gwidgets[wparam];
    if (!child->inuse)
        return;

    switch (child->type) {
    case G_TEXTWIDGET:
        if (HIWORD (lparam) == EN_CHANGE) { /* it's a text widget message */
