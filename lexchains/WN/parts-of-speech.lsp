...
(defmacro define-part-of-speech (canonical-name (file-type &rest wn-db-tokens)
  ...
  `(progn
    ...
    (push (list nil (string-downcase (symbol-name ,canonical-name))
		,canonical-name)
...
(progn
  ...
  ;(define-part-of-speech :adjective-satellite "adj" "s")
...
(defun part-of-speech-for-wordnet-db-token (token)
  (third (car (member token +part-of-speech-table+
		      :key #'first
		      :test #'string=))))
...
(defun canonicalize-part-of-speech (part-of-speech)
  (etypecase part-of-speech
    (string 
      (third (member part-of-speech +part-of-speech-table+
		     :key #'second
		     :test #'string-equal)))
    (symbol
      (third (car (member part-of-speech +part-of-speech-table+
			  :key #'third))))))
